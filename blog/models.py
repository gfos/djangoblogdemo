...
class Post(models.Model):
    ...
    # 文章正文 大段文本，使用 TextField 来存储大段文本。
    # 存储比较短的字符串可以使用 CharField
    created_time = models.DateTimeField('创建时间',default=timezone.now)
    modified_time = models.DateTimeField('修改时间')
    # 创建时间/修改时间 DateTImeField
    ...
    # 使用的是 ForeignKey，即一对多的关联关系。
    # 且自 django 2.0 以后，ForeignKey 必须传入一个 on_delete 参数用来指定当关联的
    # 数据被删除时，被关联的数据的行为，我们这里假定当某个分类被删除时，该分类下全部文章也同时被删除
    # 使用 models.CASCADE 参数，意为级联删除。
    # 因此而对于标签来说，一篇文章可以有多个标签，同一个标签下也可能有多篇文章，所以我们使用 
